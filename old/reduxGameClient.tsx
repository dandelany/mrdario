...
export const socketClusterReducer = function(state = initialState, action: Action) {
  switch (action.type) {
    case DEAUTHENTICATE:
      return {
        ...state,
...
let destructionId: number | undefined;
let destroyer;
let options = {};
let hocOptions = {};
...
export const reduxSocket = (gcOptions: GameClientOptions = {}, _hocOptions = {}) => (_ComposedComponent: Component) =>
  class SocketClustered extends Component {
    ...
    constructor(props: any, context: any) {
      ...
      options = AuthEngine ? {...gcOptions, authEngine: new AuthEngine(context.store)} : gcOptions;
      hocOptions = {...hocOptionsDefaults, ...madeHocOptions};
      socket = socketCluster.connect(options);
      destroyer = () => {
        socket.disconnect();
        socketCluster.destroy(gcOptions);
        if (onDisconnect) {
          onDisconnect(true, gcOptions, madeHocOptions, socket);
        }
        initialized = false;
      }
    }
