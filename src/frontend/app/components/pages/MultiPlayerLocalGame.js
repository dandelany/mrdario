...
import MultiPlayerLocalGameController from 'game/MultiPlayerLocalGameController';
...
class MultiPlayerLocalGame extends React.Component {
  static defaultProps = {
    cellSize: 32,
    heightPercent: .85,
    padding: 15
  ...
  _renderGame = (gameState) => {
    const hasGrid = gameState && gameState.grid;
    if(!hasGrid) return null;

    const {cellSize, match} = this.props;
    const {params} = match;
    // pass fractional padding to set padding to a fraction of cell size
    const padding = (this.props.padding < 1) ? this.props.padding * cellSize : this.props.padding;
    const numRows = gameState.grid.size || 17;
    const numCols = gameState.grid.get(0).size || 8;
    ...
    return <div {...{style, className: 'game-playfield'}}>
      <WonOverlay {...{gameState, highScores: [], rank: 1, params, style: wonOverlayStyle}} />
  ...
  render() {
    const {games, highScores, rank, pendingMode} = this.state;
    const hasGame = this.game && games && games.length;
    const {params} = this.props.match;

    return <div className="multi-game-playfield-container">
      {(pendingMode && pendingMode !== params.mode) ?
        // if game has been won or lost, redirect to the proper URL
        <Redirect push to={`/multi/local/level/${params.level}/speed/${params.speed}/${pendingMode}`}/> :
        null
      }

      {hasGame ?
        games.map(game => this._renderGame(game))
        : null
      }
...
export default withRouter(responsiveGame(MultiPlayerLocalGame));
